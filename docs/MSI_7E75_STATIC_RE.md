...
- HID static analysis confirms the primary LEDKeeper MB800 helper path uses `MsiHid.HID_Basic` P/Invokes into `Lib\MsiHid.dll`; the native wrapper statically imports Windows HID APIs including `HidD_SetFeature`.
...
| `MSI_LED.MSI_800sLed` / `MsiHid.dll` | LEDKeeper MB800 helper and native HID wrapper; opens common MSI HID VID/PID `0x0DB0:0x0076`, validates serial prefix, constructs Gen1/Gen2 feature reports, and calls `HidD_SetFeature` through `Lib\MsiHid.dll`. | High for decoded MS-7E75 zones because profile field `69` resolves to `NUC126_MB800`. | Static helper path only; no MSI binary or HID device was opened. | Current strongest zone-to-HID-report path. Still not a raw register map or ready Linux support. |
...
- Disassemble `Lib\MsiHid.dll` around `openMyDevice_Read`, `SetFeature`, `GetFeature`, and `GetAllDevicesID`.
